...
### Синхронизация порога удаления события: mock -3 → -1
Статус: выполнено (mock), **⚠️ РАСХОЖДЕНИЕ, требует решения Alex** (schema.sql)

По прямому указанию Alex: боевой порог в `vote_on_event` был вручную
изменён на `<= -1` в самой Supabase (в этой сессии, не агентом — правка
сделана напрямую в БД, вне репозитория). `src/lib/adapters/mock/events.ts`
синхронизирован: `if (ev.positiveVotes - ev.negativeVotes <= -1)` вместо
`<= -3`, чтобы mock и боевой режим не расходились (правило проекта).

**⚠️ Найдено при проверке, НЕ исправлено молча (по общему правилу проекта
"расхождения — фиксировать в лог, не чинить за рамками задачи")**:
`supabase/schema.sql` в репозитории по-прежнему содержит `<= -3` (обе
ветки — админ и обычный пользователь, функция `vote_on_event`, ~строки
291-314). То есть сейчас **три места, а не два**, и репозиторий (файл —
номинально "финальный источник правды по SQL") разошёлся с тем, что
реально выполняется в продакшене:
- Живая БД в Supabase: порог `-1` (применено вручную, вне репо).
- `supabase/schema.sql` в репозитории: всё ещё `-3`.
- `mock/events.ts`: теперь `-1` (этот заход).

Если `schema.sql` когда-нибудь будут заново применять на чистом проекте
(как описано в самом файле — "выполнить целиком в Supabase SQL Editor на
пустом проекте"), порог тихо откатится на `-3`, разойдясь с mock и с тем,
что было в бою до этого. Нужно явное решение Alex: обновить ли
`schema.sql` до `-1`, чтобы файл снова стал источником правды — агент не
стал трогать `schema.sql` в этом заходе, т.к. в задаче явно был указан
только `mock/events.ts`.
